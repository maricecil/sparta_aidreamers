...
titanic.head()  # ()안에 원하는 행을 결정함. 첫 5행은 survived, pclass, sex, age, sibsp로 나타난다.

titanic.describe()  # 기본적인 통계확인 # 수치형 값들의 통계 값을 확인할 수 있다. 그래서 나머지 칼럼들은 보이지 않는다.
...
std_values = titanic_statistics.loc['std']  # std : 표준편차(평균을 기준으로 분포된 값으로, 값이 작을수록 밀집도가 높다)
...
print(count_values) # 빈 값이 없는 값 중 가장 큰 값은 'survived'등으로 보여진다. 891명의 생존여부가 모두 기록되어 있다는 뜻이다. 한편 'age'는 714개로 177개의 결측치가 존재한다.
print(std_values)  # 평균으로부터 분포된 값 중 가장 큰 값은 'fare'로 49.69로 보여진다. 이는 49파운드만큼 퍼져있어 가격차가 많이 났다는 뜻이다. 
# 하지만 평균 값은 32파운드였고, 75%는 31파운드 이내로 값을 지불해 대부분은 낮은 요금으로 탑승했음을 알 수 있다.
...
print(titanic.isnull().sum())  #isnull()은 결측치를 true로 나타내는데, sum()을 통해 개수를 쉽게 확인한다.
...
# fillna함수를 통해 embarked의 결측치를 mode(최빈값, 가장 많이 사용된 값)으로 채웠다. 
# 최빈값으로 바꾸는 mode임에도 [0]을 넣는 이유는 여러 최빈 값이 존재할 수 있기 때문이다.
# 두 번째로 낮은 최빈값으로 바꿀 경우 value_count()로 빈도를 계산하여 내림차순으로 바꾸어 확인할 수 있다.
# inplace=true 공식으로 원본 데이터를 수정해 embarked에 적용했다.
print(titanic['age'].isnull().sum()) # age열에서 결측치가 중앙값으로 채워져 0으로 출력된다. 
print(titanic['embarked'].isnull().sum()) # embarked열에서 결측치가 최빈값으로 채워 0으로 출력된다.
...
X = titanic.drop('survived', axis=1) # 독립변수(feature), 생존자를 예측하고자 survived를 제외한 나머지 열을 x에 저장함
y = titanic['survived'] # 종속변수, 목표(target)로 하는 survived 열을 y에 저장함


from sklearn.model_selection import train_test_split  # 데이터 훈련, 테스트 분류
from sklearn.preprocessing import StandardScaler  #  데이터평균을 0, 분산을 1로 스케일링
from sklearn.linear_model import LogisticRegression  # 로지스틱 회귀 모델 생성 
from sklearn.metrics import accuracy_score, classification_report  # 정확도 계산, 분류 보고서 생성, 혼동행령 생성
...
# 데이터 스케일링
scaler = StandardScaler()  # standerdscaler로 독립변수(feature)를 평균 0, 표준편차 1로 정규화 시킴(상대평가)
# 정규화도 데이터 값을 0~1로 압축해주지만 모든 데이터를 백분율로 변환한다는 차이가 있음(최저 0, 최고 1)
X_train = scaler.fit_transform(X_train)  # 학습데이터에 적용  # fit()은 훈련 데이터의 평균과 표준편차를 계산해줌
X_test = scaler.transform(X_test)  # 테스트데이터에도 동일하게 적용


# 모델 생성 및 학습
model = LogisticRegression()  # 로지스틱 회귀 모델 생성  
#logisticregression(max_iter=200)으로 기본값을 두어 최상의 가중치를 찾는 방법도 있음
...
# 실제값(target)과 예측값(predict)을 받아서 각 클래스별로 평가지표를 계산한 후 출력
# 정확도 외의 정밀도, 재현율, F1-score 등 성능을 상세히 보여줌

# 예측한 클래스 중 실제 클래스와 일치한 데이터 비율인 정밀도(precision)는 0.82로, 82%가 사망하였음을 보여줌
# 실제 클래스 중 예측한 클래스와 일치한 비율인 재현율(recall)은 0.86으로, 86% 확률로 사망자를 놓치지 않고 잘 찾아냈다는 것을 보여줌
...
# classifier(분류기)는 평가방법으로 정확도(accuracy)를 사용한다. regressor(회귀)는 평가방법으로 대표적으로 mse를 사용한다.
# 해당모델은 분류기(정확도)를 쓰고 있어 mse는 알 수 없다.
