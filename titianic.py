...
# 각 열의 가장 작은 값은 'age'로 0.42로 보여진다. 0.42살은 약 5개월 되는 아기도 탑승했었다는 뜻이다.
print(min_values)
# 각 열의 가장 큰 값은 'fare'로 512.3292로 보여진다. 약512파운드를 지불한 승객이 있었음을 드러낸다.
print(max_values)
# 빈 값이 없는 값 중 가장 큰 값은 'survived'등으로 보여진다. 891명의 생존여부가 모두 기록되어 있다는 뜻이다.
print(count_values)
# 평균으로부터 퍼져 있는 값 중 가장 큰 값은 'fare'로 49.69로 보여진다. 이는 49파운드만큼 퍼져있어 가격차가 많이 났다는 뜻이다.
print(std_values)
# 1사분위가 가장 큰 값은 'age'로 20.1250으로 보여진다. 이는 25%의 승객이 20세 이하라는 것을 나타낸다.
print(q1_values)
# 중앙값이 가장 큰 값도 'age'로 28로 보여진다. 이는 50%의 승객이 28세 이하라는 것을 나타낸다.
print(median_values)
# 3사분위가 가장 큰 값도 'age'로 38세로 보여진다. 이는 75%의 승객이 38세 이하라는 것을 나타낸다.
print(q3_values)
...
# embarked열에서 결측치가 최빈값(가장 많이 사용된 값)으로 채워 0으로 출력된다.
...
# 두번째승객('0'Cherbourg)을 제외한 네 명의 승객은 '2'Southampton에서 탑승했다.
...
# 즉, 다섯 승객 중 세번째, 다섯번째 승객이 혼자 탑승했고, 나머지 승객은 합승했음을 알 수 있다.
...
# 독립변수(feature), survived를 제외한 나머지 열을 x에 저장함
...
predicted_survival_rate = (
    predicted_survived_count / total_passengers) * 100  # 생존율
